...
%Ecuaciones
    function f = f_M(P_2,P_1,K_m,n)
        f = (P_1 + P_2^n + K_m^n -  sqrt((P_1 + P_2^n + K_m^n)^2 - 4*P_1*P_2^n))/2;
        %f = P_1*((P_2^n)/(P_2^n + K_m^n));
    end

    dwc1 = k(1) + k(2)*WCCn + k(3)*laWCC - kd(1)*wc1;
    dwc2 = k(4)/(1 + K(1)*WCCn) + k(5)*FFCn - kd(2)*wc2;
    dWC1 = k(6)*wc1 + k(7)*f_M(wc1,FFCp,K(2),1) - k(9)*WC1*WC2 - kd(3)*WC1;
    dWC2 = k(8)*wc2 - kd(4)*WC2 - k(9)*WC1*WC2;
    dWCCc = k(9)*WC1*WC2 - kd(5)*WCCc -  k(10)*WCCc;
    dWCCn = k(10)*WCCc + k(11)*laWCC - kd(6)*WCCn - L*WCCn - k(13)*WCCn - k(12)*f_M(WCCn,FFCn,K(3),n);
    dlaWCC = L*WCCn + k(13)*WCCn - kd(7)*laWCC - k(11)*laWCC - k(14)*f_M(laWCC,VVDn,K(4),n);
    dfrq =  (k(15)*(K(6)*WCCn)^n + k(16)*(K(5)*laWCC)^n)/((K(5)*K(6))^n + (K(6)*WCCn)^n + (K(5)*laWCC)^n) - kd(8)*frq;
    ...
    dvvd = (k(21)*laWCC^n)/(K(7)^n + laWCC^n) - kd(13)*vvd;
